...
class College_online_routine_model extends CI_Model
{
  ...
  public function get_count_routine_list($operator_id)
  {
    $college_id = $this->session->userdata('stake_details_id_fk');
  ...
  {
    $status_code = array('4', '9', '7', '8');
    $query = $this->db->select('routine.routine_manage_id_pk,routine.online_routine_status_current,routine.routine_online_status')
      ...
      ->where_in('status_code', $status_code)
      ->limit($limit, $offset)
  ...
  public function get_all_routine_list($college_id, $operator_id, $days_name, $discipline_type, $stud_year, $semester, $period_no, $emp_id, $routine_mode)
  {
  ...
  {
    return $this->db->set('routine_online_status', '2')
  ...
  public function updateRoutineonlineusingid($routine_manage_id_pk, $college_id)
  {
    return $this->db->set('routine_online_status', '1')
      ->set('online_routine_status_current', '9')
      ->set('routine_online_assign_time', date('Y-m-d H:i:s'))
      ->set('routine_online_assign_by', $college_id)
      ->set('routine_online_assign_ip', $this->input->ip_address())
      ->where(
  ...
  public function updateRoutineonlinenotusingid($routine_manage_id_pk, $college_id)
  {
    return $this->db->set('routine_online_status', '0')
      ->set('routine_online_status_not_time', date('Y-m-d H:i:s'))
      ->set('routine_online_status_not_by', $college_id)
      ->set('routine_online_status_not_ip', $this->input->ip_address())
      ->where(
        array(
          'MD5(CAST(routine_manage_id_pk AS character varying))=' => $routine_manage_id_pk
        )
  ...
  public function forwardRoutineonlinenotusingid($routine_manage_id_pk)
  {
    return $this->db->set('online_routine_forward_time', date('Y-m-d H:i:s'))
      ->set('online_routine_forward_by', $this->session->userdata('stake_holder_login_id_pk'))
      ->set('online_routine_forward_ip', $this->input->ip_address())
      ->set('online_routine_status_current', '7')
      ->where(
        array(
          'MD5(CAST(routine_manage_id_pk AS character varying))=' => $routine_manage_id_pk
        )
