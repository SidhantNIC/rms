...
<?php } else{?>
	<?php echo form_open(base_url('admin/admin_college_employee/admin_college_employee/confirm_approval_emp'), array('id'=>'admin_approval_emp_form'))?>
        <div class="row">
        	<div class="col-md-12">
            ...
            <div class="col-sm-12">
                <span style="color:blue; font-size:16px; font-weight:bold;">Do you want to active the employee</span>
            </div>
        </div>
...
<?php if($this->input->method(TRUE) == 'POST'){ ?>
	<?php if(isset($code) && $code == 1){ ?>
        ...
        $(document).ready(function(evt){
				...
				$(".confirm_approval_emp").remove();
//				$("#wationg_approved_btn_"+clg_hash).remove();
                //$("#approved").show();
				//$("#clg_temp_status_approved_"+clg_hash).html('Approved');
                //$("#clg_status_"+clg_hash).html('<a class="btn-sm btn-success">Approved</a>');
                //$(".pending_"+clg_hash).remove();
